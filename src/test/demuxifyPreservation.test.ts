...
$>*/
/*<#*/
import { CounterQualities, CounterState, createCounterConcept } from '../concepts/counter/counter.concept';
...
const noise = (muxium: Muxium<any>, final: boolean, done: jest.DoneCallback) => {
  type Deck = {
    ...
    muxified: Concept<any, any> & Concept<CounterState, CounterQualities>,
  }
    ...
    return [
      ...(() => {
        const stages = new Array(10).fill(stage(() => {
          //
        })).map((_, i) => stage(({d, dispatch, concepts}) => {
          console.log(`${i}: ${ID} One`, d.one.k.state(concepts));
          console.log(`${i}: ${ID} Counter`, d.counter.k.count.select());
      ...
      stage(({d, stagePlanner, concepts}) => {
        console.log(`FINAL: ${ID} One`, d.one.k.state(concepts));
        console.log(`FINAL: ${ID} Counter`, d.counter.k.count.select());
        console.log(`FINAL: ${ID} Muxified`, d.muxified.k.count.select());
        expect(d.counter.k.count.select()).toBe(d.muxified.k.count.select());
        if (final) {
          setTimeout(() => {
            done();
          }, 1000);
...
test('Demuxify Preservation', (done) => {
